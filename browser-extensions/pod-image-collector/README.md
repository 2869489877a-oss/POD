...
- 已内置 OSS 上传配置，把已选图片直接上传到 `oss://wms-europe-file/杨文韬文件/`，不依赖 POD 后台。
...
- 定时采集：支持在 Temu / SHEIN / Pinterest 三个网站中选择一个，填写目标网址、每次数量和间隔天/小时/分钟，浏览器保持打开时会自动采集并上传 OSS。
- 图片加载等待：下拉、点击更多或翻页后统一等待 3 秒，再读取图片 URL，减少抓到低清晰度占位图的情况。
- JPG 统一保存：本地下载和 OSS 上传都会先把图片转换成 JPG；透明图片会使用白色背景。
...
8. 勾选需要保存的图片。
9. 点击“下载选中图片”。
10. 如果要上传 OSS，填写“员工姓名”，点击“上传到 OSS”。
...
```text
oss://wms-europe-file/杨文韬文件/赵凯/4-23/temu/001-xxx.webp
oss://wms-europe-file/杨文韬文件/赵凯/4-23/shein/001-xxx.webp
```
...
定时任务触发后，插件会自动打开目标网址，在后台标签页采集图片并上传 OSS，完成后关闭该标签页。上传路径仍然是：

```text
oss://wms-europe-file/杨文韬文件/员工姓名/北京时间日期/网站类型/图片文件
```
