...
import { failAssetDeleteJob } from "@/lib/assets/delete-jobs";
...
export async function POST(request: Request) {
  ...
  try {
    const job = await failAssetDeleteJob(
      createSupabaseServiceRoleClient(),
      jobId,
      typeof body.error === "string" && body.error.trim() ? body.error.trim() : "Asset delete worker failed",
    );
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to mark asset delete job failed", ok: false },
      { status: 500 },
    );
  }
}
