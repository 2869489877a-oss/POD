...
async function listTemplates(includeArchived: boolean) {
  ...
  return templates.map((template) =>
    normalizeTemplate(
      template,
      sources.filter((source) => source.template_id === template.id),
    ),
  );
}
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "读取采集模板失败", templates: [] },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  if (input.sources.length > 0) {
    ...
    if (sourceError) {
      await supabase.from("image_collection_templates").delete().eq("id", template.id);
  ...
  return NextResponse.json({
    ...
    templates: await listTemplates(false),
  });
}
